package br.com.adalbertofjr.hotel.repository.sqlite
...
class ProviderRepository(val ctx: Context) : HotelRepository {
    override fun save(hotel: Hotel) {
        val uri = ctx.contentResolver.insert(
            HotelKotlinProvider.CONTENT_URI,
            getValues(hotel)
        )
    ...
    override fun remove(vararg hotels: Hotel) {
        hotels.forEach { hotel ->
            val uri = Uri.withAppendedPath(
                HotelKotlinProvider.CONTENT_URI, hotel.id.toString()
            )
    ...
    override fun hotelById(id: Long, callback: (Hotel?) -> Unit) {
        val cursor =
            ctx.contentResolver.query(
                Uri.withAppendedPath(HotelKotlinProvider.CONTENT_URI, id.toString()),
                null, null, null, null
            )
    ...
    fun getValues(hotel: Hotel): ContentValues {
        val contentValues = ContentValues()
        if (hotel.id > 0) {
            contentValues.put(COLUMN_ID, hotel.id)
        }

        contentValues.put(COLUMN_NAME, hotel.name)
        contentValues.put(COLUMN_ADDRESS, hotel.address)
        contentValues.put(COLUMN_RATING, hotel.rating)
        return contentValues
    }

    fun hotelFromCursor(cursor: Cursor): Hotel {
        ...
        return Hotel(id, nome, address, rating)
    }
}